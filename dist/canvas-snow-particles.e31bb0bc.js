...
function main() {
  ...
  stage.addEventListener("mousemove", function (e) {
    ...
    var maxDistance = calculateDistance(bRect.x + bRect.width / 2, 0, bRect.y + bRect.height / 2, 0);
    var relPos = getRelativePosition(MousePos[0], center[0], MousePos[1], center[1]);
    var slope = calculateSlope(relPos[0], 0, relPos[1], 0);
    var quadrant = determineQuadrant(relPos[0], relPos[1]);
    var distancePercent = percentageOfMaxDistance(calculateDistance(relPos[0], 0, relPos[1], 0), maxDistance);
    var intersection = getIntersection(slope, quadrant);
    mouseMoveAnimations(intersection, Math.abs(slope) > 2 ? 2 * (Math.abs(slope) / slope) : slope);
  });
  setStage();
  setSize(canvas);
  var buffer = createCanvasBuffer(canvas);
  var particles = initializeParticles(canvas);
  resize(canvas, stage);
...
function calculateTranslationAmount(proportionalComponent, proportionFactor, slopeMagnitude) {
  var shim = 0; // if (debug) {
  //     // console.log(
  ...
  if (slopeMagnitude && slopeMagnitude > 1) {
    ...
    shim = 40 * (slopeMagnitude * 0.3) * (Math.abs(proportionalComponent) / proportionalComponent);
  }
...
function getIntersection(slope, quadrant) {
  ...
  var x = 1 / (1 + Math.pow(slope, 2));
  var y = x * slope;
  console.log(quadrant, slope);

  if (slope > 0) {
    return quadrant === 1 ? [Math.abs(x), Math.abs(y)] : [-Math.abs(x), -Math.abs(y)];
  } else {
    return quadrant === 2 ? [-Math.abs(x), Math.abs(y)] : [Math.abs(x), -Math.abs(y)];
  }
}

var eyeEntrance = function eyeEntrance() {
  _gsap.default.set("#eye-open", {
    y: "-50%"
  });
...
var createCanvasBuffer = function createCanvasBuffer(canvas) {
...
var setStage = function setStage() {
  ...
  _gsap.default.set(".tree-2", {
    scaleX: 0.6,
    y: 200
  });

  window.onload = function () {
    _gsap.default.to(".tree", {
      delay: 3,
      rotate: -3,
      scaleX: 1,
      scaleY: 0.9,
...
var resize = function resize(canvas, stage) {
  ...
  stage.style.height = "".concat(window.innerWidth * 0.5625, "px");
  stage.style.width = "".concat(window.innerWidth, "px");
};

var setSize = function setSize(canvas) {
...
function rectifyCeleration(value, particle) {
  if (value > 0.5 || value < -0.5) {
    ...
    var changedValue = value < 0 ? -(Math.abs(value) - 0.5 / 400 * DISPLACEMENT_FORCE_MAX) : Math.abs(value) - 0.5 / 200 * DISPLACEMENT_FORCE_MAX;
...
var initializeParticles = function initializeParticles(canvas) {
  var particles = [];
...
function animation(ctx, canvas, particles, buffer) {
  ...
  buffer.ellipse(canvas.width / 2, canvas.height, canvas.width / 1.5, canvas.width / 12, 0, 0, Math.PI * 2);
  buffer.stroke();
  buffer.fill();
  ...
  for (var i = LENGTH - 1; i > -1; i--) {
    ...
    if (particles[i].x > 0 || particles[i].x < canvas.width || particles[i].y > 0 || particles[i].y < canvas.height) {
      buffer.arc(particles[i].x, particles[i].y, i % 2 === 0 ? 1 : 2, 0, Math.PI * 2);
    }

    buffer.stroke();
    buffer.fill();
    checkIfInMouseRange(particles[i]);
    particles[i].y > canvas.height ? particles[i] = createParticle(canvas) : particles[i] = {
...
main();
},{"gsap":"node_modules/gsap/index.js"}],"../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
